...
import (
	...
	blueprintpkg "github.com/cloudwego/eino/compose/blueprint"
	schemad "github.com/cloudwego/eino/schema/declarative"
	skillpkg "github.com/cloudwego/eino/skill"
)

// Resolver resolves task refs into runtime runnables.
type Resolver struct {
	Documents           blueprintpkg.DocumentLoader
	ComponentFactory    schemad.ComponentFactory
	SkillLoader         skillpkg.SpecLoader
	SkillAssembler      skillpkg.Assembler
	InterpreterResolver blueprintpkg.InterpreterResolver
}

// NewResolver creates a job resolver backed by shared declarative infrastructure.
func NewResolver(documents blueprintpkg.DocumentLoader, factory schemad.ComponentFactory, skillLoader skillpkg.SpecLoader, skillAssembler skillpkg.Assembler, interpreter blueprintpkg.InterpreterResolver) *Resolver {
	return &Resolver{
		Documents:           documents,
...
func (r *Resolver) ResolveGraph(ctx context.Context, ref schemad.Ref) (compose.AnyGraph, error) {
	...
	case schemad.RefKindBlueprintDocument:
		...
		loader := &blueprintpkg.Loader{Documents: r.Documents}
		bp, err := loader.LoadGraph(ctx, ref)
		if err != nil {
			return nil, err
		}
		builder := &blueprintpkg.Builder{
			Loader:              loader,
			ComponentFactory:    componentFactoryAdapter{factory: r.ComponentFactory},
			InterpreterResolver: r.InterpreterResolver,
		}
		return builder.BuildAnyGraph(ctx, bp)
...
func (r *Resolver) resolveBlueprintNode(ctx context.Context, node *schemad.NodeSpec) (any, error) {
	...
	case schemad.NodeKindComponent:
		if node.Component == nil {
			return nil, fmt.Errorf("component node %s is missing component spec", node.Key)
		}
		return schemad.BuildComponent(ctx, node.Component, r.ComponentFactory, componentResolverAdapter{resolver: r.InterpreterResolver})
...
func normalizedTargetType(targetType string, refKind string) string {
	...
	switch refKind {
	case schemad.RefKindBlueprintDocument, schemad.RefKindInterpreterGraph:
...
type componentResolverAdapter struct {
	resolver blueprintpkg.InterpreterResolver
}
...
type componentFactoryAdapter struct {
	factory schemad.ComponentFactory
}

func (a componentFactoryAdapter) BuildComponent(ctx context.Context, spec *schemad.ComponentSpec) (any, error) {
	if a.factory == nil {
		return nil, fmt.Errorf("component factory is required")
	}
	return a.factory.BuildComponent(ctx, spec)
}
