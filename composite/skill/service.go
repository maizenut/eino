...
// BuildGraphExecutor assembles a loaded skill spec and adapts its graph into an executor.
func (s *Service) BuildGraphExecutor(ctx context.Context, spec *SkillSpec, opts ...compose.GraphCompileOption) (*GraphExecutor, error) {
	runnable, err := s.Build(ctx, spec)
	if err != nil {
		return nil, err
	}
	return NewGraphExecutor(runnable, opts...)
}
...
// BuildGraphExecutorByName resolves a skill from the registry and adapts its graph into an executor.
func (s *Service) BuildGraphExecutorByName(ctx context.Context, name string, opts ...compose.GraphCompileOption) (*GraphExecutor, error) {
	if s == nil || s.Registry == nil {
		return nil, fmt.Errorf("skill registry is required")
	}
	if name == "" {
	...
	return s.BuildGraphExecutor(ctx, spec, opts...)
}
...
// LoadAndBuildGraphExecutor loads a skill from an external target and adapts its graph into an executor.
func (s *Service) LoadAndBuildGraphExecutor(ctx context.Context, target string, opts ...compose.GraphCompileOption) (*GraphExecutor, *SkillSpec, error) {
	spec, err := s.Load(ctx, target)
	if err != nil {
		return nil, nil, err
	}
	executor, err := s.BuildGraphExecutor(ctx, spec, opts...)
	if err != nil {
		return nil, nil, err
	}
	return executor, spec, nil
}
